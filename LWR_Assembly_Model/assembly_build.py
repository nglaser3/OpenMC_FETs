...
fuel_outter_radius = 0.5
zirc_outter_radius = 0.025 + fuel_outter_radius
...
left, right = (
    openmc.XPlane(x0=-pitch * num_pins, boundary_type="reflective"),
    openmc.XPlane(x0=pitch * num_pins, boundary_type="reflective"),
)
back, front = (
    openmc.YPlane(y0=-pitch * num_pins, boundary_type="reflective"),
    openmc.YPlane(y0=pitch * num_pins, boundary_type="reflective"),
)
boundingrectangle = openmc.Cell(
    region=+left & -right & +back & -front & -top & +bottom, fill=lattice
)
...
zernike_filter = openmc.ZernikeFilter(
    order=zern_ord, r=fuel_outter_radius, x=_x0, y=_y0
)
...
model.settings.source = openmc.IndependentSource(
    space=openmc.stats.Box(lower_left=ll, upper_right=ur, only_fissionable=True)
)
