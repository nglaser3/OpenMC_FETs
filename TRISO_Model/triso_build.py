...
for file in glob.glob(
    "*.xml"
):  # removes all current .xml files just to start from scratch
    os.remove(file)
...
radii = [
    ...
    510e-4,
    546.7e-4,
]  # outer radii for fuel, buffer, ipyc, and SiC
spheres = [openmc.Sphere(r=radius) for radius in radii]
cells = [
    openmc.Cell(fill=UCO, region=-spheres[0]),
...
top, bottom = (
    openmc.ZPlane(z0=compact_height / 2, boundary_type="reflective"),
    openmc.ZPlane(z0=-compact_height / 2, boundary_type="reflective"),
)
compact_region = -compact_surface & -top & +bottom
centroids = openmc.model.pack_spheres(
    radius=outer_radius, region=compact_region, pf=pf, seed=seed
)

# bounding cells (hexagon / cylinder)
outterhex = openmc.model.HexagonalPrism(
    edge_length=2 * compact_radius, boundary_type="reflective", corner_radius=0
)
hexagonal_compact = openmc.Cell(
    region=-outterhex & +bottom & -top & +compact_surface, fill=SiC
)
cylinder_compact = openmc.Cell(region=compact_region)

# lattice generation
trisos = [
    openmc.model.TRISO(outer_radius=outer_radius, fill=triso_univ, center=centroid)
...
lattice = openmc.model.create_triso_lattice(
    trisos=trisos, lower_left=lowerleft, pitch=pitch, shape=shape, background=Carbon
)
...
Settings 
"""
trigger = openmc.Trigger("std_dev", 0.0001)
...
TRISO.settings.source = openmc.IndependentSource(
    space=openmc.stats.Box(
        [-compact_radius, -compact_radius, -compact_height],
        [compact_radius, compact_radius, compact_height],
        only_fissionable=True,
    )
)
TRISO.settings.keff_trigger = {"type": "std_dev", "threshold": 0.0001}
